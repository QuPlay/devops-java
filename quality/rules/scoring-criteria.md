...
9. 死代码（合并阻断项）— 以下任一情况必须清理后才能提交：
   - **未使用的局部变量**：方法体内声明但从未读取的变量（重构后遗留最常见）
   - **未使用的方法参数**：参数在方法体内从未引用（接口约定的除外，如 `@Override` 方法）
   - **未使用的 private 方法**：类内声明但无任何调用点的私有方法
   - **未使用的 import**：导入但未引用的类（IDE 灰色标记）
   - **未使用的字段**：类中声明但从未读写的字段（`@Autowired` / `@Resource` 注入后未使用同样算）
   - 死代码是重构不彻底的标志，降低可读性且误导后续维护人员
